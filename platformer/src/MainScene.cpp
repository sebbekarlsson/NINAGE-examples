...
void MainScene::init(float delta) {
    for (int x = 0; x < 10; x++) {
        this->instantiate(new Block(x * 70, app->getHeight() - 70.0f));
    }

    for (int x = 11; x < (11+10); x++) {
        this->instantiate(new Block(x * 70, app->getHeight() + (70*2)));
    }

    this->instantiate(new Player(70, app->getHeight() - (70 * 3)));
}
